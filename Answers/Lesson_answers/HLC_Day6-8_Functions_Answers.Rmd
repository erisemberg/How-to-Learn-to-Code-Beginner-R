...
abbr <- sapply(days, function(x) {
  # if the first character is T or S, extract first two characters
  if(substr(x, 1, 1) == "T" | substr(x, 1, 1) == "S") {
    substr(x, 1, 2)
  } else { # everything else, extract first character
    substr(x, 1, 1)
  } 
})
```
