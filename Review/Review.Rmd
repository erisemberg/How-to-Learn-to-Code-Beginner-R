...
Arguments with default values do not need to be specified when running a function, but they can be overwritten if you would like to specify a value other than the default.
...
We learned how to create our own functions in R. R functions can be created with the generic structure shown below:
...
As an example, we can create an example R function to take the average of two numbers:
```{r}
avg_two <- function(number1, number2) {
  avg_val <- (number1 + number2) / 2
  return(avg_val)
}
```
